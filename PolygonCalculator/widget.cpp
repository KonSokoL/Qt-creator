...
void Widget::calculateAll(){
    ...
    ui->squareEdit->setText(ui->pointList->count() < 3  ? "не существует" :
                                                          (crossed ? "Самопересекающийся многоугольник" :
                                                                     (s == 0 ? "Вырожденный многоугольник" :
                                                                               QString::number(s, 'f', 2))));
    ui->perimeterEdit->setText(ui->pointList->count() < 3 ? "Многоугольника": QString::number(p, 'f', 2));
}
...
{
    if (!(ui->pointList->selectedItems().count() > 0))
    {
        return;
    }
    bool ok = true;
